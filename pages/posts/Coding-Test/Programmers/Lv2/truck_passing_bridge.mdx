...
function solution(bridge_length, weight, truck_weights) {
    ...
    while(true){
        answer++;
        if(endTime.includes(answer)){
            endTime.shift();
            crossing.shift();
        }
        if(truck_weights.length===0 && crossing.length===0) break;
        let crossing_weight =0;
        for(let i=0 ; i<crossing.length;i++){
            crossing_weight+=crossing[i];
        }
        if(crossing_weight+truck_weights[0]<=weight){
            crossing.push(truck_weights.shift())
            endTime.push(answer+bridge_length)
        }
    }
    return answer;
}
...
테스트 2 〉	통과 (8.21ms, 36.8MB)
테스트 3 〉	통과 (0.15ms, 33.4MB)
테스트 4 〉	통과 (9.49ms, 36.9MB)
테스트 5 〉	통과 (15.26ms, 36.8MB)
테스트 6 〉	통과 (11.73ms, 36.6MB)
테스트 7 〉	통과 (0.34ms, 33.4MB)
테스트 8 〉	통과 (0.18ms, 33.4MB)
테스트 9 〉	통과 (3.44ms, 36.7MB)
