...
function solution(cacheSize, cities) {
    ...
    const citieslen= cities.length;
		...
		if(cacheSize===0) return citieslen*5
    
    for(let i=0; i<citieslen;i++){
        let turn = cities.shift().toUpperCase();
        if(caches.includes(turn)){
            answer++;
            caches.splice(caches.indexOf(turn),1)
        }else{
            answer+=5;
        }
...
테스트 18 〉	통과 (0.66ms, 33.5MB)
테스트 19 〉	통과 (0.53ms, 33.5MB)
테스트 20 〉	통과 (0.98ms, 33.5MB)
채점 결과
정확성: 100.0
합계: 100.0 / 100.0
```